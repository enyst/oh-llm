...
tmux split-window -h -t "$SESSION:0" "ls -1t \"$RUNS_DIR\" 2>/dev/null | head; exec $SHELL"
tmux split-window -v -t "$SESSION:0.1" "tail -F \"$(ls -1td \"$RUNS_DIR\"/* 2>/dev/null | head -n1)/logs/run.log\" 2>/dev/null || true; exec $SHELL"
...
- If you override runs dir, set `OH_LLM_RUNS_DIR` (or run `oh-llm run --runs-dir ...`).
...
- `uv run oh-llm run --runs-dir /tmp/oh-llm-runs --profile demo --json`
